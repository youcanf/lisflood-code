...
def loadmap(name,pcr=False, lddflag=False, timestampflag='exact', averageyearflag=False):
    ...
    if not(load):
        ...
        try:
            ...
            targets = list()
            for it in varNames:
                if not it == "lon" and not it == "lat" and not it == 'x' and not it == 'y' and not it == 'laea' and not it == "lambert_azimuthal_equal_area" and not it == 'time':
                    targets.append(it)
    ...
    if flag == 0:
        ...
        if flagTime:
            ...
            if 'x' in metadataNCDF.keys():
                ...
                # CM write as float64
                value = nf1.createVariable(prefix, 'd', ('time', 'y', 'x'), zlib=True, fill_value=-9999,
                                           chunksizes=(1, row, col))
                # value = nf1.createVariable(prefix, fillval, ('y', 'x'), zlib=True,fill_value=-9999)
            if 'lon' in metadataNCDF.keys():
                value = nf1.createVariable(prefix, 'd', ('time', 'lat', 'lon'), zlib=True, fill_value=-9999,
                                           chunksizes=(1, row, col))
        else:
            if 'x' in metadataNCDF.keys():
